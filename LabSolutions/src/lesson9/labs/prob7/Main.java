...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		Function<Integer, Integer> byBigger = n -> Math.abs(n);
		Function<Integer, Integer> bySign = n -> (n > 0 ? 1 : -1);

		System.out.println(list.stream().sorted(Comparator
				.comparing(byBigger)
				.thenComparing(bySign)).collect(Collectors.toList()));
	...
	public static void ordering2(List<String> words) {
		Function<String, String> byReverse = n -> (new StringBuilder(n).reverse().toString());

		System.out.println(words.stream().sorted(Comparator
				.comparing(byReverse)).collect(Collectors.toList()));
